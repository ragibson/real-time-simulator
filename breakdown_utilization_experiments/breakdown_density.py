...
def multiprocessor_breakdown_density(scheduler, task_system, utilization_tolerance=1e-3):
    @functools.lru_cache(maxsize=10)
    def test_weight(weight):
        reweighted_task_system = reweight_task_system(weight, task_system)
        _, schedulable = scheduler.generate_schedule(reweighted_task_system)
    ...
    weight = (scheduler.num_processors + len(task_system) / min(task.period
                                                                for task in task_system)) / task_system.utilization()
    weight_step = weight
